...
- 中英文查询停用词过滤和领域词扩展。
- 比较问题拆分为独立主题检索；结果按主题交错，避免一个概念填满全部 Top-K。
- 定义型缩写保留精确锚点，例如 `MCP` 与 `Model Context Protocol` 连续出现时优先定义证据。
- 原始 query 中出现已知紧凑术语时，要求展开后的组成词全部出现在同一 Chunk；普通多词 query 不启用该严格规则。
...
- 比较问题的结果为了保证主题覆盖会按主题交错，因此同一列表中的相邻结果不一定按全局分数降序；应结合 `comparison_topics`，分别检查每个主题的 Hit@K、Recall@K 和 MRR。
- 紧凑术语规则只针对原始输入中的 `agentruntime`、`agentloop`、`toolcalling` 等写法；它不会把所有自然语言查询都提升为 100% 词项覆盖，避免误伤 `Claude Code 自定义工具` 这类证据只包含主题词的有效 Chunk。
...
SQLite FTS5 是当前主流程的运行时后端，但索引构建和索引使用是两个动作：

- `build-index` 负责扫描知识库、切分 Chunk 和重建 SQLite 数据库。
- Chat、Eval 和 `compare-search` 只读打开已经构建的数据库。
- 知识库更新后不自动重建，用户需要显式再次运行 `build-index`。
- 缺少索引或 schema 版本不兼容时，程序直接提示构建命令，不静默回退到内存检索。

这样可以避免每次启动都处理约 4 万个 Chunk，也让索引内容是否对应当前知识库成为一个明确、可管理的工程步骤。
